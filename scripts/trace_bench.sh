...
HEADER_GLOBAL_LIST=`cat $TA_RESULTS_PATH/$GLOBAL_LIST | grep $PREF_AVG`
if [ x"$HEADER_GLOBAL_LIST" == "x" ]; then
	echo "${PREF_AVG}_$PREFIX#$TITLE_AVG#$XLAB_AVG#$YLAB_AVG" >> $TA_RESULTS_PATH/$GLOBAL_LIST
fi
HEADER_GLOBAL_LIST=""

HEADER_GLOBAL_LIST=`cat $TA_RESULTS_PATH/$GLOBAL_LIST | grep $PREF_SCHED`
if [ x"$HEADER_GLOBAL_LIST" == "x" ]; then
	echo "${PREF_SCHED}_$PREFIX#$TITLE_SCHED#$XLAB_SCHED#$YLAB_SCHED" >> $TA_RESULTS_PATH/$GLOBAL_LIST
fi
HEADER_GLOBAL_LIST=""

HEADER_GLOBAL_LIST=`cat $TA_RESULTS_PATH/$GLOBAL_LIST | grep $PREF_MIGR`
if [ x"$HEADER_GLOBAL_LIST" == "x" ]; then
