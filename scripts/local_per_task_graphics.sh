...
Generate "local per_task" graphic, namely a graphic built using data
...
while getopts "hf:d:t:" optionName; do
...
# numbers of title, xlab, ylab are always equal
...
for i in `seq $AR_SIZE`; do 
	...
	for d in $DIM_LIST_GRAPH; do
		for j in $TASK_LIST_GRAPH; do
			...
			echo -n "${AVG_TAG}<$d><$j>	" >> $GRAPH_FOLDER/$GRAPH_FILE
			...
			# rows are tagged with <$d><$j>,  then call calc_stat to compute
			# average of theese numbers 

			cat $STATS_FILE | grep "<${d}><$j>" | \
	...
	for d in $DIM_LIST_GRAPH; do
		for j in $TASK_LIST_GRAPH; do
			echo -n "${VAR_TAG}<$d><$j>	" >> $GRAPH_FOLDER/$GRAPH_FILE
